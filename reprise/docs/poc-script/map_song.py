...
def clean_subtitles(vtt_text):
    ...
    for line in lines:
        if "-->" in line or "WEBVTT" in line or "Kind:" in line or "Language:" in line:
            continue
        # Remove timestamps and positioning tags
        line = re.sub(r'<\d{2}:\d{2}:\d{2}\.\d{3}>', '', line)
        line = re.sub(r'<[^>]+>', '', line).strip()
        
        # Prevent repeating lines (common in auto-captions)
...
def get_song_metadata(url):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            # --- 1. RE-INTEGRATED SQUARE ICON LOGIC ---
            thumbnails = info.get('thumbnails', [])
            icon_url = None

            # Look for native square thumbnails (width approx equals height)
            square_thumbs = [
            ...
            if square_thumbs:
                ...
                # Fallback: force square if it's a Google/YT Music hosted image
            ...
            if target_lang:
                ...
                if res.status_code == 200:
                    lyrics_text = clean_subtitles(res.text)

            return {
                "song_name": info.get('track') or info.get('title'),
                "artist": info.get('artist') or info.get('uploader'),
                "icon_url": icon_url, # Thumbnails are back!
